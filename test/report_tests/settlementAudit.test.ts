    ...
    it('Run a transfer, settle a settlement and extract the report', async () => {
      ...
      await workbook.xlsx.readFile(reportFile)
        .then(() => {
          ...
          const payerStartingBalance = parseFloat(payerFirstFundsIn[9].replace(',', ''))
          ...
          expect(payerFundsOut[8].replace(',', '')).toEqual(transferAmount) // funds in amount
          expect(payerFundsOut[9].replace(',', '')).toEqual(balanceAfterFundsOut) // balance

          // Validate the first funds in for payee
          const payeeFirstFundsIn = payeeRecords[payeeFundsInTransferId]
          const payeeStartingBalance = parseFloat(payeeFirstFundsIn[9].replace(',', ''))
          ...
          expect(payeeFirstFundsIn[7].replace(',', '')).toEqual(fundsInAmount) // funds in amount
          ...
          expect(payeeFundsIn[7].replace(',', '')).toEqual(transferAmount) // funds in amount
          expect(payeeFundsIn[8]).toBeUndefined() // funds out amount
          expect(payeeFundsIn[9].replace(',', '')).toEqual(balanceAfterFundsIn) // balance
        })
